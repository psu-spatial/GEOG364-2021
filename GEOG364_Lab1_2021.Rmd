...
**Make sure that there is a code chunk where you have loaded at least these 3 packages. These need to be in a code chunk before you start using the commands inside them. So if you skipped the step above, you need these in your script**
...
We’re going to work with a table of data that’s already pre-loaded into R inside the ggplot2 package. First, type the `?mpg` command in the console. This will bring up the help file. If it can't find the dataset, you need to load the ggplot2 package using the library command.
...
```{r, echo=FALSE}
knitr::include_graphics("GEOG364_Lab1_fig01_Submit.png")
```
